...
std::vector<double> ProcessNormalization::getKappa(const char *name){
    if (thetaList_.find(name)){
        const double logKappa = logKappa_.at(thetaList_.index(name));
        return std::vector<double>{std::exp(logKappa)};
    }
    if (asymmThetaList_.find(name)){
        const std::pair<double, double> logKappas = logAsymmKappa_.at(asymmThetaList_.index(name));
        return std::vector<double>{std::exp(logKappas.first), std::exp(logKappas.second)};
    }
