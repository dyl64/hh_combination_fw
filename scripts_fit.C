...
int printSummary(ModelConfig *m_mc, RooAbsData *m_data);
...
int fit() {
    ...
    TFile* file = new TFile("./output/combined/spin0/A-bbbb_bbtautau_bbyy-nocorr/4000_with_Asimov_POI_0_NP_fit.root");
    //TFile* file = new TFile(".//output/rescaled/spin0/bbbb/4000_with_Asimov_POI_0_NP_fit.root");
...
int printSummary(ModelConfig *m_mc, RooAbsData *m_data)
{
    ...
    const RooArgSet* m_gobs = m_mc->GetGlobalObservables(); assert(m_gobs);
