...
from combination_xml import create_combination_xml

class TaskBase:
    
    WSC_PATH  = os.environ['WORKSPACECOMBINER_PATH']
    RST_PATH = os.environ['ROOSTATPATH']
    ...
    def initialize(self, resonant_type, poi_name, data_name, do_better_bands=True, CL=0.95, blind=True, 
                   mass_expr=None, param=None, new_method=False, verbose=False, minimizer_options=None,
    ...
    def sanity_check(self):
        ...
        if not os.path.exists(self.RST_PATH):
            raise FileNotFoundError('RooStatTools directory {} does not exist.'.format(self.RST_PATH))
        if self.new_method:
            try:
                import quickstats
    ...
    def new_limit_setting(self, param_point):
    ...
    def limit_setting(self, param_point):
        if self.parameterized_points is None:
            ...
            ws_with_Asimov_NP_fit_path = os.path.join(self.asimov_dir, asimov_NP_fit_filename)
            basename = param_point["basename"]
            ls.CalcLimit_exp_and_obs(basename, ws_with_Asimov_NP_nom_path, ws_with_Asimov_NP_fit_path, 
                                     self.rootfiles_dir, self.do_better_bands, self.data_name, self.CL, self.blind, 
                                     self.poi_name, self.snapshot)
            ws_with_Asimov_NP_nom_contents_path = ws_with_Asimov_NP_nom_path.replace('.root','.contents')
            ws_with_Asimov_NP_fit_contents_path = ws_with_Asimov_NP_fit_path.replace('.root','.contents')
            dg.writeWScontentToFile(ws_with_Asimov_NP_nom_path, ws_with_Asimov_NP_nom_contents_path)
            dg.writeWScontentToFile(ws_with_Asimov_NP_fit_path, ws_with_Asimov_NP_fit_contents_path)
        else:
            filename = self.BASIS_WS_PATTERN.format(**param_point)
            basis_ws_path = os.path.join(self.basis_dir, filename)
            basename = param_point["basename"]
            ls.CalcLimit_param(basename, basis_ws_path, self.rootfiles_dir, self.do_better_bands, 
                               self.data_name, self.CL, self.blind, self.parameterized_points, self.poi_name,
                               self.snapshot, parallel=self.parallel)
    ...
    def finalize(self, param_points):
        if (self.parameterized_points is None) and (not self.new_method):
            print('Extracting limits from {0}'.format(self.rootfiles_dir))
            isSM = self.resonant_type == 'nonres'
            datafile_path = os.path.join(self.rootfiles_dir, self.datafile_name)
            ls.get_exp_and_obs_limit(self.rootfiles_dir, scaling='no', output_dat=datafile_path, 
                                     isSM=isSM, blind=self.blind)
        if self.new_method:
...
class TaskPipelineWS(TaskBase):
    ...
    def initialize(self, input_dir, output_dir, resonant_type, channel, scaling_release, 
                   ...
                   CL=0.95, blind=True, mass_expr=None, param=None, new_method=False, verbose=False, 
        ...
        super().initialize(resonant_type, new_poiname, new_dataname, do_better_bands, CL, blind, 
                           mass_expr, param, new_method=new_method, verbose=verbose,
    ...
    def rescale(self, param_point):
        ...
        wsc_rescale_config_template = os.path.join(self.RST_PATH, 'workspaceCombiner', 'rescale.xml')
    ...
    def _run_pipeline(self, param_point):
        ...
        if not self.do_limit:
            return None
        if self.new_method:
            self.new_limit_setting(param_point)
        else:
...
class TaskCombination(TaskBase):
    ...
    CHANNEL_WS_EXPR = r'{basename}_with_Asimov_POI_0_NP_nom.root'
    CHANNEL_WS_EXPR_NEW = r'{basename}.root'
    ...
    def get_combination_xml(self, param_point):
        ...
        input_ws_paths = {}
        channel_ws_expr = self.CHANNEL_WS_EXPR if not self.new_method else self.CHANNEL_WS_EXPR_NEW
