...
with open("process_commands.sh", 'w') as f:
    for process_channels_cmd_str in process_channels_commands:
        ...
        f.write("Arguments = "+process_channels_cmd_str.replace(" ", "____").replace("\"", "").replace("skip-likelihood", "do-likelihood"))
        f.write("Queue 1\n")
...
with open("combine_commands.sh", 'w') as f:
    for combine_ws_cmd_str in combine_ws_commands:
        ...
        f.write("Arguments = "+combine_ws_cmd_str.replace(" ", "____").replace("\"", "").replace("skip-likelihood", "do-likelihood"))
        f.write("Queue 1\n")
