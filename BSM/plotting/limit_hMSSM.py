...
# f_exp=interpolate.CloughTocher2DInterpolator(list(zip(x,y)), z_exp, fill_value=0)
# f_obs=interpolate.CloughTocher2DInterpolator(list(zip(x,y)), z_obs, fill_value=0)
...
fhMSSM=TFile.Open("../LHCHWG_ROOT/hMSSM_13.root","r")
f_mH=fhMSSM.Get("m_H")
f_widthH=fhMSSM.Get("width_H")
f_ggFH=fhMSSM.Get("xs_gg_H_fix")
f_H_hh=fhMSSM.Get("br_H_hh_fix")
...
for i in range(nmA):
    for j in range(ntanB):
        ...
        if(mH>=251):
            ...
            # if(abs(mA-407.4)<0.1):
            #     print(mA,tanB,ggFH,br_H_hh,xsec_limit_obs,Z_obs[i,j])
        else:
            Z_exp[i,j]=0
            Z_obs[i,j]=0
        if(np.isnan(Z_exp[i,j])):Z_exp[i,j]=Z_obs[i,j]=0

        if (br_H_hh==0): br_H_hh=1e-6
...
# ax.scatter(X[Z_obs<1],Y[Z_obs<1],Z_obs[Z_obs<1],label=r'95CL >$\Delta$ nll > 90CL',c='#9b59b6', alpha=0.9)
...
ax2.text(0.03,0.35,r'$H \to hh, h\mathrm{MSSM}$',family='Helvetica',size='13')
...
obsnm=mlines.Line2D([],[],color=combedgecolor,linewidth=2)
...
if plot_single_ch:
    ...
    legend2=[obsnm,expnm,bbtautauexpnm,bbbbexpnm,bbyyexpnm]
    labels2=['Observed','Expected',r'$\Gamma_H/m_H>5$%']
    ...
    legpaper=[combnm,expnm]
    labelspaper=['Observed','Expected']
