...
class UpperLimit1DPlot(AbstractPlot):
    ...
    def get_default_legend_order(self):
        return ["observed", "expected", "third", "one_sigma", "two_sigma", "curve0", "curve1"]        
    ...
    def draw_curve(self, ax, data, i):
        ...
        self.update_legend_handles({f'curve{i}': handles})
    ...
    def draw(self, logx:bool=False, xlabel:Optional[str]=None, markersize:float=50.,
        ...
        for i, category in enumerate(self.category_df):
            ...
            # draw expected
            expected_limit = df['0']
            expected_handle = ax.vlines(expected_limit, i, i + 1, colors=self.color_pallete['expected'], linestyles='dashed', linewidth=2,
        ...
        xlim = ax.get_xlim()
        ax.set_xlim(xlim[0] - (xlim[1]/0.8 - xlim[1])*0.5, xlim[1]/0.8)
        ...
        if self.curve_data is not None:
            for i, curve in enumerate(self.curve_data):
                self.draw_curve(ax, curve, i)
        ...
        self.add_legend_decoration(border_leg, targets=["one_sigma", "two_sigma", "curve0"])
