...
double scale_poi           = 1.75*1;  // zoom the impact axis
double scale_theta         = 1.75*1;  // zoom the pull axis; 3.4 for 1lep...1.7 for everybody else
...
void drawPlot_pulls2(string prepath, TCanvas* c1, TPad* pad1, TPad* pad2);
void ROOT2Ascii(string working_dir, string type);
...
//void drawPlot_pulls(string working_dir = "", bool remakeAscii = 0, bool postFitOrder = true, string mass = "", string datasetName = "", string plotName = "")
void drawPlot_pulls(string working_dir = "", bool remakeAscii = 0, bool postFitOrder = true, string mass = "", string datasetName = "", string plotName = "", string channelname = "" )
{
    gStyle->SetHatchesLineWidth(hatch_width);

    string cardName = working_dir;
    //string output_name = working_dir + "./ranking";
    ...
    if (remakeAscii) {
      ROOT2Ascii(working_dir, "breakdown_add");
      ROOT2Ascii(working_dir, "pulls");
      //return;
    }
    ...
    drawPlot_pulls2(working_dir, c1, pad1, pad2);
    ...
    // -- Call the actual plotting script
    //drawPlot_pulls2(cardName, output_name, c1, pad1, pad2);
    ...
    t3.SetNDC();
    TString lumitex2 = lumi2 + " fb^{-1}"; // Updated luminosity style
    TString stex = "#sqrt{s} = " + energy2 + " TeV, " + lumitex2;//, #int Ldt = " + lumi2 + " fb^{-1}";
    t3.DrawLatex(channelPosX, channelPosY, stex.Data());

    TLatex t2;
    t2.SetTextSize(0.025);
    t2.SetNDC();
    t2.DrawLatex(channelPosX, channelPosY-0.035, (channelname).c_str());
    t2.DrawLatex(channelPosX, channelPosY-0.070, (mass+", "+datasetName).c_str());

    //string figure_path;
    //figure_path = working_dir;
    //if (!m_postFitOrder)
    //saveName << "prefit_";

    c1->Update();
 
    c1->SaveAs( ("./output/pdf/"+plotName+"_Ranking.pdf").c_str() );
    c1->SaveAs( ("./output/png/"+plotName+"_Ranking.png").c_str() );
}


// The actual plotting goes on here
void drawPlot_pulls2(string working_dir, TCanvas* c1, TPad* pad1, TPad* pad2)
{
    gStyle->SetHatchesLineWidth(hatch_width);

    // - Load and initialize ascii files
    ifstream testFile((working_dir+"/ascii/pulls.txt").c_str());
    if (testFile.fail()) {
        cout << "ERROR::file " << (working_dir+"ascii/pulls.txt").c_str() << "does not exist.";
        exit(1);
    }

    fileHolder pulls;
    drawPlot(working_dir+"/ascii/pulls.txt", 8, pulls);

    fileHolder pulls_ol;

    // - Load and initialize the normalisation factor ascii file
    ifstream testFile2((working_dir+"/ascii/pulls_nf.txt").c_str());
    if (testFile2.fail()) {
        cout << "ERROR::file " << (working_dir+"/ascii/pulls_nf.txt").c_str() << "does not exist.";
        exit(1);
    }

    fileHolder nfs;
    drawPlot((working_dir+"/ascii/pulls_nf.txt"), 8, nfs);

    fileHolder nfs_ol;

    // load and initialize the category uncertainties
    //cout<<"CHECK OUT "<< working_dir+"/ascii/breakdown_add.txt"<<endl;
    ifstream testFile3((working_dir+"/ascii/breakdown_add.txt").c_str());
    if (testFile3.fail()) {
        cout << "ERROR::file " << (working_dir+"/ascii/breakdown_add.txt").c_str() << "does not exist.";
        exit(1);
    }

    fileHolder cats;
    drawPlot(working_dir+"/ascii/breakdown_add.txt", 3, cats);

    fileHolder cats_ol;

    if (overlay != "") drawPlot(working_dir+"/ascii/breakdown_add.txt", 3, cats_ol);
    ...
    Int_t nlines = 0;
    ifstream idFile((working_dir+"/ascii/pulls_id.txt").c_str());
    while (true) {
    ...
    if (overlay != "") {
        Int_t nlines_ol = 0;
        ifstream idFile_ol((working_dir+"/ascii/"+overlay+"_pulls_id.txt").c_str());
        while (true) {
    ...
    Int_t nf_nlines = 0;
    ifstream idFile2((working_dir+"/ascii/pulls_nf_id.txt").c_str());
    while (true) {
    ...
    if (overlay != "") {
        ...
        ifstream idFile2_ol((working_dir+"/ascii/"+overlay+"_pulls_nf_id.txt").c_str());
        while (true) {
    ...
    Int_t cats_nlines = 0;
    ifstream idFile3((working_dir+"/ascii/breakdown_add_id.txt").c_str());
    while (1) {
    ...
    if (overlay != "") {
        ...
        ifstream idFile3_ol((working_dir+"/ascii/"+overlay+"_breakdown_add_id.txt").c_str());
        while (1) {
    ...
    for(int ii=0; ii<nrNuis; ii++) poi_zero.push_back(0.);
    ...
    for (int i = offset; i < nrNuis+offset; i++){
        if(labels[i-offset].find("norm")!= string::npos)
            isNorm.push_back(1);
        else if(labels[i-offset].find("alpha_r16_LowHtCR")!= string::npos)
            isNorm.push_back(1);
        else if(labels[i-offset].find("alpha_r16_HighHtCR")!= string::npos)
            isNorm.push_back(1);
        else if(labels[i-offset].find("nbkg_bb_bbyy")!= string::npos)
            isNorm.push_back(1);
        else if(labels[i-offset].find("nbkg_bj_bbyy")!= string::npos)
            isNorm.push_back(1);
        else if(labels[i-offset].find("slope_bb_bbyy")!= string::npos)
            isNorm.push_back(1);
        else if(labels[i-offset].find("slope_bj_bbyy")!= string::npos)
            isNorm.push_back(1);
        else if(labels[i-offset].find("c1_WWyy")!= string::npos)
            isNorm.push_back(1);
        else if(labels[i-offset].find("c2_WWyy")!= string::npos)
            isNorm.push_back(1);
        else if(labels[i-offset].find("nConBkgSR_WWyy")!= string::npos)
            isNorm.push_back(1);
        else isNorm.push_back(0);
    ...
    TLegend* leg = makeLeg();
    leg->SetX1(channelPosX + 0.32);
    leg->SetY1(channelPosY-0.0775);
    leg->SetX2(channelPosX + 0.82);
...
void ROOT2Ascii(string working_dir, string type)
{
    ...
    system( ("mkdir -vp " + working_dir + "/ascii/").c_str());

    string root_folder = type;
    enum ConvertMode {pulls, breakdown};

    ConvertMode mode;
    if (root_folder.find("pulls") != string::npos) {
        mode = pulls;
    } else if (root_folder.find("breakdown") != string::npos) {
        mode = breakdown;
        //mode = pulls;
    } else {
    ...
    // start to convert
    bool ignoreInfNan = true;

    std::vector<TString> list;
    string root_folder_path = working_dir + "/root-files/" + root_folder;

    TSystemDirectory dire(root_folder_path.c_str(), root_folder_path.c_str());
    ...
    while((file = (TSystemFile*) next())) {
        ...
        cout<<"The file name in the directory - "<<root_folder_path<< " - "<<fname<<endl;
        list.push_back(fname.Data());
    }
   
    // the type is exactly the mode +_add for breakdown
    // the outFileName is exactly part of the dir to save the output 
    stringstream outFileName;
    outFileName << working_dir << "/ascii/" << type;

    // each time to call ROOT2Ascii will create 4 files
    // ( one to numbers in the histogram produced by runPulls.C/runBreakdown.C, one to the names of the nuiP ) *2 
    // if you split the nuiP by normF and real nuiP in the WS as below.
    
    // the nuisance parameters 
    ofstream outFile((outFileName.str()+".txt").c_str());
    ofstream outFile_id((outFileName.str()+"_id.txt").c_str());

    // save the normalisation factors if you specify them below - isNorm = true
    ...
    for (vector<TString>::iterator it = list.begin(); it != list.end(); it++) {
        ...
        fileName << root_folder_path << "/" << *it;

        cout<<"What is this: "<<*it<<endl;
	  ...
	  cout << "ERROR: Could not open file " << fileName.str() << endl;
        ...
        if (histoName.Contains("nConBkgSR_WWyy")) isNorm = true;
        ...
        // if (histoName.Contains("ATLAS_norm")) isNorm = true;
	...
	// if (histoName.Contains("VH_")) isNorm = true;
        ...
        // check the bin if it's infinite or nan.
        ...
        if (ignoreInfNan && isInfNan) {
         ...
         // give the pulls.txt or breakdown.txt the index of nuiP or normF
         // it doesn't matter what is nrNPs / nrNFs next line
        (isNorm? outFile_nf : outFile) << (isNorm? nrNFs++ : nrNPs++) << " ";
        if (isNorm) cout<<"The number of the normalisation factors: "<<nrNFs<<endl;
        else cout<<"The number of not normalisation factors / the nuisance parameters: "<<nrNPs<<endl;
        //(isNorm? outFile_nf : outFile) << (isNorm? nrNPs++ : nrNFs++) << " "; // liq

        //  this will have a "\n" here to save the next nuisance parameter to next line
        for (int bin = 1; bin <= nrBins; bin++) {
            double number = hist->GetBinContent(bin);
            (isNorm? outFile_nf : outFile) << number << ((bin < nrBins)?" ":"\n");
        }

        // save both the constrained and the unconstrained nuisance parameters' names into the id files
...
TString translateNPname(TString internalName, bool isMVA)
{
  ...
  if(internalName == "alpha_SysWMbb_B0_WbbORcc") {
    if(isMVA) return "#splitline{W+b#bar{b}, W+c#bar{c} m_{jj} shape}{    (p_{T}^{V} < 120 GeV)}";
    else return "#splitline{W+b#bar{b}, W+c#bar{c} m_{jj} shape}{    (p_{T}^{V} < 90 GeV)}";
  }
  ...
  if(internalName == "alpha_SysWblWbbRatio_B0"){
    if(isMVA) return "#splitline{W+bl to W+b#bar{b} normalisation}{    (p_{T}^{V} < 120 GeV)}";
    else return "#splitline{W+bl to W+b#bar{b} normalisation}{    (p_{T}^{V} < 90 GeV)}";
  }
